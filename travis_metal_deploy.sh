...
REQUESTUUID=$(xcrun altool --notarize-app -t osx -f ~/KingStation_Metal.dmg --primary-bundle-id libretro.KingStation -u $APPLE_ID -p $APPLE_ID_PASS -itc_provider ZE9XE938Z2 | awk '/RequestUUID/ { print $NF; }')
sleep 200
...
rsync -avhP -e 'ssh -p 12346 -o StrictHostKeyChecking=no' ~/KingStation_Metal.dmg travis@bot.libretro.com:~/nightly/apple/osx/x86_64/
